...
| Pooling through a known mean shift | A single origin that ignores the break is the wrong information set | Known-break Chow; sup-F search | Search T_b on the estimation window only | `tests/test_corrections.py::test_sup_chow_search_stays_inside_the_estimation_window` | Andrews p-values for the sup |
